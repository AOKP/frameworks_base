...
public class RibbonGestureCatcherView extends LinearLayout {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        if (mRibbonSwipeStarted) {
                            ...
                            for (int k = 0; k < historySize + 1; k++) {
                                ...
                                if (mVerticalLayout) {
                                    distance =
                                            mRightSide ? (mDownPoint[0] - x) : (x - mDownPoint[0]);
                                } else {
    ...
    public Animation PlayOutAnim() {
        if (mDragButton != null) {
            Animation animation = AnimationUtils
                    .loadAnimation(mContext, com.android.internal.R.anim.slow_fade_out);
    ...
    public Animation PlayInAnim() {
        if (mDragButton != null) {
            Animation animation = AnimationUtils
                    .loadAnimation(mContext, com.android.internal.R.anim.slow_fade_in);
            animation.setStartOffset(0);
            animation.setAnimationListener(new Animation.AnimationListener() {
                @Override
                public void onAnimationStart(Animation animation) {
    ...
    private void updateLayout() {
        ...
        if (mVerticalLayout) {
            ...
            dragSize = ((mScreenWidth) * (mButtonWeight * 0.02f)) /
                    getResources().getDisplayMetrics().density;
    ...
    protected void updateSettings() {
        ...
        mDragButtonOpacity = Settings.System
                .getInt(cr, Settings.System.RIBBON_DRAG_HANDLE_OPACITY[ribbonNumber], 0);
        mButtonWeight = Settings.System
                .getInt(cr, Settings.System.RIBBON_DRAG_HANDLE_WEIGHT[ribbonNumber], 30);
        mButtonHeight = Settings.System
                .getInt(cr, Settings.System.RIBBON_DRAG_HANDLE_HEIGHT[ribbonNumber], 50);
        mLongSwipeAction =
                Settings.System.getString(cr, Settings.System.RIBBON_LONG_SWIPE[ribbonNumber]);
        if (TextUtils.isEmpty(mLongSwipeAction)) {
            mLongSwipeAction = AwesomeConstant.ACTION_APP_WINDOW.value();
            Settings.System.putString(cr, Settings.System.RIBBON_LONG_SWIPE[ribbonNumber],
                    AwesomeConstant.ACTION_APP_WINDOW.value());
        }
        mLongPressAction =
                Settings.System.getString(cr, Settings.System.RIBBON_LONG_PRESS[ribbonNumber]);
        if (TextUtils.isEmpty(mLongPressAction)) {
            mLongPressAction = AwesomeConstant.ACTION_APP_WINDOW.value();
            Settings.System.putString(cr, Settings.System.RIBBON_LONG_PRESS[ribbonNumber],
                    AwesomeConstant.ACTION_APP_WINDOW.value());
        }
        vib = Settings.System
                .getBoolean(cr, Settings.System.SWIPE_RIBBON_VIBRATE[ribbonNumber], false);
        mLocation = Settings.System
                .getInt(cr, Settings.System.RIBBON_DRAG_HANDLE_LOCATION[ribbonNumber], 0);
        updateLayout();
    }
}
