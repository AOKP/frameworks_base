...
public class AokpSwipeRibbon extends LinearLayout {
    ...
    private boolean mText, hasNavBarByDefault, NavBarEnabled, navAutoHide;
    private int mHideTimeOut = 5000;
    private int mAnimDelay = 390;
    private boolean showing = false;
    private boolean animating = false;
    private int mSize, mColor, mOpacity, animationIn, animationOut, mIconLoc;
    ...
    public void hideRibbonView() {
        if (mPopupView != null && !animating && showing) {
            animating = true;
            PlayOutAnim();
            mHandler.postDelayed(removePopup, mAnimDelay);
    ...
    public void createRibbonView() {
        ...
        BackgroundAlphaColorDrawable bacd = new BackgroundAlphaColorDrawable(
            mColor > 0 ? mColor : Color.BLACK);
        float opacity = (255f * (mOpacity * 0.01f));
        bacd.setAlpha((int)opacity);
        ...
        mRibbonMain.setBackground(bacd);
        switch (mIconLoc) {
            case 0:
    ...
    private void setupRibbon() {
        ...
        if (mLocation.equals("bottom")) {
            ...
            mRibbon.addView(sv);
            if (!navAutoHide && (NavBarEnabled || hasNavBarByDefault)) {
                ...
                mRibbon.setPadding(1,adjustment,1,1);
            } else {
                mRibbon.setPadding(1,1,1,1);
            }
        }
    }
   ...
   protected void updateSettings() {
        ...
        mColor = Settings.System.getInt(cr,
                 Settings.System.SWIPE_RIBBON_COLOR, 0);
        ...
        mEnableSides[0] = mEnableSides[0] && !navAutoHide && !(NavBarEnabled || hasNavBarByDefault);
        createRibbonView();
    }
