...
public class UnicornCaseView extends FrameLayout {
    private static final String TAG = UnicornCaseView.class.getSimpleName();
    ...
    private static final int[] ALPHABETS = {
            R.drawable.unicorns_A,
            R.drawable.unicorns_O,
            R.drawable.unicorns_K,
            R.drawable.unicorns_P,
    };
    ...
    private static final int NUM_PASTRIES = PASTRIES.length + ALPHABETS.length + UNICORNS.length;
    ...
    public UnicornCaseView(Context context) {
        this(context, null);
    }

    public UnicornCaseView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public UnicornCaseView(Context context, AttributeSet attrs, int defStyle) {
...
//        return 0xFF000000 | (int) (Math.random() * (float) 0xFFFFFF); // totally random
    ...
    public synchronized void fillFreeList(int animationLen) {
        ...
        while (! mFreeList.isEmpty()) {
            ...
            if (which < 0.05f) {
                ...
                d = mDrawables.get(pick(ALPHABETS));
            } else if (which < 0.7f) {
                d = mDrawables.get(pick(UNICORNS));
            } else {
                d = null;
            }
            if (d != null) {
    ...
    public static class RescalingContainer extends FrameLayout {
        ...
        public void setView(UnicornCaseView v) {
            addView(v);
            mView = v;
        }
        ...
        public void setLightness(float p) {
            mLightness = p;
            getLightness();
            final int x = (int) (p * 0xff);
            setBackgroundColor(x << 24 & 0x00000000);
        }
