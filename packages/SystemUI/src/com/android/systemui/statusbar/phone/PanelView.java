...
public class PanelView extends FrameLayout {
        ...
        public void computeCurrentVelocity(long timebase) {
            ...
            for (final Iterator<MotionEventCopy> iter = mEventBuf.descendingIterator();
                ...
                if (last != null) {
                    ...
                    if (FlingTracker.DEBUG) {
                        Slog.v("FlingTracker",
                                String.format("   [%d] dx=%.1f dy=%.1f dt=%.0f vx=%.1f vy=%.1f",
                                        i,
                                        dx, dy, dt,
                                        (dx / dt),
                                        (dy / dt)
    ...
    private void runPeekAnimation() {
        if (DEBUG) {
            LOG("peek to height=%.1f", mPeekHeight);
        }
    ...
    private void loadDimens() {
        ...
        mPeekHeight = res.getDimension(R.dimen.peek_height)
                ...
                - (mHandleView == null ? 0 :
                mHandleView.getPaddingTop()); // the handle might have a topshadow
    ...
    // Pass all touches along to the handle, allowing the user to drag the panel closed from its
    // interior
    @Override
                ...
                public boolean onTouch(View v, MotionEvent event) {
                    ...
                    switch (event.getAction()) {
                        ...
                        case MotionEvent.ACTION_MOVE:
                            ...
                            if (mBrightnessSliderEnabled && mTracking &&
                                    mShouldReactToBrightnessSlider) {
                                if (mPropFactor == null) {
                                    setPropFactor();
                                }
                                mBrightnessValue =
                                        checkMinMax(Math.round(event.getRawX() * mPropFactor));
                                changeBrightness();
                            }
                            if (!mJustPeeked) {
                        ...
                        case MotionEvent.ACTION_CANCEL:
                            ...
                            if (mVelocityTracker != null) {
                                ...
                                if (xVel > mFlingGestureMaxXVelocityPx) {
                                    xVel =
                                            mFlingGestureMaxXVelocityPx; // limit how much we
                            ...
                            if (DEBUG) {
                                LOG("gesture: dy=%f vel=(%f,%f) vlinear=%f",
                                        deltaY,
                                        xVel, yVel,
                                        vel);
                            }
    ...
    public void fling(float vel, boolean always) {
        if (DEBUG) {
            LOG("fling: vel=%.3f, this=%s", vel, this);
        }
        mVel = vel;
    ...
    public void setExpandedHeight(float height) {
        if (DEBUG) {
            LOG("setExpandedHeight(%.1f)", height);
        }
        mRubberbanding = false;
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        if (DEBUG) {
            LOG("onLayout: changed=%s, bottom=%d eh=%d fh=%d", changed ? "T" : "f", bottom,
                    (int) mExpandedHeight, mFullHeight);
        }
    ...
    public void setExpandedHeightInternal(float h) {
        ...
        if (!(mRubberbandingEnabled && (mTracking || mRubberbanding)) && h > fh) {
            h = fh;
        }
        mExpandedHeight = h;

        if (DEBUG) {
            LOG("setExpansion: height=%.1f fh=%.1f tracking=%s rubber=%s", h, fh,
...
//        setLayoutParams(lp);
    ...
    private float getFullHeight() {
        if (mFullHeight <= 0) {
            ...
            measure(MeasureSpec.makeMeasureSpec(android.view.ViewGroup.LayoutParams.WRAP_CONTENT,
                    MeasureSpec.EXACTLY),
                    MeasureSpec.makeMeasureSpec(android.view.ViewGroup.LayoutParams.WRAP_CONTENT,
                            MeasureSpec.EXACTLY));
        }
        return mFullHeight;
    }
