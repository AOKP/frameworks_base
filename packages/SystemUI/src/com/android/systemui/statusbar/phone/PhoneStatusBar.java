...
public class PhoneStatusBar extends BaseStatusBar {
    ...
    private static final int INTRUDER_ALERT_DECAY_MS = 0; // disabled, was
                                                          // 10000;

    private static final boolean CLOSE_PANEL_WHEN_EMPTIED = true;

    private static final int NOTIFICATION_PRIORITY_MULTIPLIER = 10; // see
    ...
    private float mFlingGestureMaxOutputVelocityPx; // how fast can it really
                                                    // go? (should be a little
                                                    // faster than
    ...
    NotificationPanelView mNotificationPanel; // the sliding/resizing panel
    ...
    private boolean mSuppressStatusBarDrags; // while a cling is up, briefly
                                             // deaden the bar to give things
                                             // time to settle

    boolean mAnimating;
    boolean mClosing; // only valid when mAnimating; indicates the initial
                      // acceleration
    float mAnimY;
    float mAnimVel;
    float mAnimAccel;
    ...
    // ensure quick settings is disabled until the current user makes it through
    // the setup wizard
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        ...
        if (ENABLE_INTRUDERS) {
            mIntruderAlertView = (IntruderAlertView) View.inflate(context, R.layout.intruder_alert,
                    null);
            mIntruderAlertView.setVisibility(View.GONE);
            mIntruderAlertView.setBar(this);
        }
        if (MULTIUSER_DEBUG) {
            mNotificationPanelDebugText = (TextView) mNotificationPanel
                    .findViewById(R.id.header_debug_info);
        ...
        if (mSettingsButton != null) {
            ...
            if (mHasSettingsPanel) {
                if (mStatusBarView.hasFullWidthNotifications()) {
                    ...
                    // there is a settings panel, but it's on the other side of
                    // the (large) screen
        ...
        if (mHasFlipSettings) {
            mNotificationButton = (ImageView) mStatusBarWindow
        ...
        mScrollView.setVerticalScrollBarEnabled(false); // less drawing during
                                                        // pulldowns
        ...
        mSbBatteryController = (SbBatteryController) mStatusBarView
                .findViewById(R.id.battery_cluster);
        mNetworkController = new NetworkController(mContext);
        mBluetoothController = new BluetoothController(mContext);

        SignalClusterView signalCluster = (SignalClusterView) mStatusBarView
                .findViewById(R.id.signal_cluster);
        ...
        if (mShowCarrierInPanel) {
            ...
            // for mobile devices, we always show mobile connection info here
            // (SPN/PLMN)
        ...
        if (mHasSettingsPanel) {
            ...
            if (mHasFlipSettings) {
                // a version of quick settings that flips around behind the
                // notifications
                ...
                if (settings_stub != null) {
                    ...
                    mSettingsPanel = (SettingsPanelView) mStatusBarWindow
                            .findViewById(R.id.settings_panel);
                }

                if (mSettingsPanel != null) {
                    if (!ActivityManager.isHighEndGfx()) {
                        mSettingsPanel.setBackground(new FastColorDrawable(context.getResources()
                                .getColor(
                                        R.color.notification_panel_solid_background)));
                    }
                }
            ...
            mQS.setControllers(mBluetoothController, mNetworkController, mBatteryController,
                    mLocationController, null);
            mQS.setContainer((LinearLayout) mNotificationPanel.findViewById(R.id.quick_toggles),
                    XzibitToggler.STYLE_TRADITIONAL);

            if (mSettingsContainer != null) {
                mQS.setContainer(mSettingsContainer, XzibitToggler.STYLE_TILE);
                if (!mNotificationPanelIsFullScreenWidth) {
        ...
        public boolean onTouch(View v, MotionEvent event) {
            if (event.getAction() == MotionEvent.ACTION_OUTSIDE) {
                ...
                if (mGesturePanel != null) {
                    // possible that we disabled AutoHide, but still have this
                    // listener
                    // hanging around.
                    hideNavBar();
                }
    ...
    private void showNavBar() {
        if (mWindowManager != null && !mAutoHideVisible) {
            ...
            // Start the timer to hide the NavBar; <-- I LOLZ. Mike even
            // comments in java. ;-)
        }
    }
    ...
    private void hideNavBar() {
        if (mNavigationBarView != null && !mLockscreenOn) {
            try {
                ...
                // we are probably in a state where NavBar has been created, but
                // not actually added to the window
                Log.d("PopUpNav", "Failed Removing NavBar");
            }
            try {
                // we remove the GesturePanel just to make sure we don't have a
                // case where we are
    ...
    protected void onBarTouchEvent(MotionEvent ev) {
        // NavBar/SystemBar reports a touch event - reset the hide timer if
        // applicable
        if (mNavBarAutoHide && mAutoHideTimeOut > 0) {
            mHandler.removeCallbacks(delayHide); // reset
            mHandler.postDelayed(delayHide, mAutoHideTimeOut);
    ...
    private void repositionNavigationBar() {
        ...
        if (!mNavBarAutoHide || (mNavBarAutoHide && mAutoHideVisible)) {
            // we only want to update the NavBar if we know it's attached to the
            // window.
    ...
    private void addIntruderView() {
        WindowManager.LayoutParams lp = new WindowManager.LayoutParams(
                ...
                WindowManager.LayoutParams.TYPE_STATUS_BAR_PANEL, // above the
                                                                  // status bar!
                WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN
    ...
    public void addNotification(IBinder key, StatusBarNotification notification) {
        ...
        /*
         * DISABLED due to missing API if (ENABLE_INTRUDERS && ( //
         * TODO(dsandler): Only if the screen is on
         * notification.notification.intruderView != null)) { Slog.d(TAG,
         * "Presenting high-priority notification"); // special new transient
         * ticker mode // 1. Populate mIntruderAlertView if
         * (notification.notification.intruderView == null) { Slog.e(TAG,
         * notification.notification.toString() +
         * " wanted to intrude but intruderView was null"); return; } // bind
         * the click event to the content area PendingIntent contentIntent =
         * notification.notification.contentIntent; final View.OnClickListener
         * listener = (contentIntent != null) ? new
         * NotificationClicker(contentIntent, notification.pkg,
         * notification.tag, notification.id) : null;
         * mIntruderAlertView.applyIntruderContent
         * (notification.notification.intruderView, listener);
         * mCurrentlyIntrudingNotification = notification; // 2. Animate
         * mIntruderAlertView in mHandler.sendEmptyMessage(MSG_SHOW_INTRUDER);
         * // 3. Set alarm to age the notification off (TODO)
         * mHandler.removeMessages(MSG_HIDE_INTRUDER); if
         * (INTRUDER_ALERT_DECAY_MS > 0) {
         * mHandler.sendEmptyMessageDelayed(MSG_HIDE_INTRUDER,
         * INTRUDER_ALERT_DECAY_MS); } } else
         */
    ...
    protected void updateCarrierLabelVisibility(boolean force) {
        if (!mShowCarrierInPanel)
            return;
        // The idea here is to only show the carrier label when there is enough
        // room to see it,
        ...
        final boolean makeVisible =
                !(emergencyCallsShownElsewhere && mNetworkController.isEmergencyOnly())
                        && mPile.getHeight() < (mNotificationPanel.getHeight()
    ...
    protected void setAreThereNotifications() {
        ...
        if (mHasFlipSettings
                ...
                && mScrollView.getVisibility() != View.VISIBLE) {
            // the flip settings panel is unequivocally showing; we should not
            // be shown
            mClearButton.setVisibility(View.INVISIBLE);
    ...
    void makeExpandedVisible(boolean revealAfterDraw) {
        ...
        // Expand the window to encompass the full screen in anticipation of the
        // drag.
        // This is only possible to do atomically because the status bar is at
        // the top of the screen!
        WindowManager.LayoutParams lp = (WindowManager.LayoutParams) mStatusBarWindow
                .getLayoutParams();
        ...
        // Kick off the reveal animation after this is complete to avoid
        // animation latency.
        if (revealAfterDraw) {
    ...
    public void flipToNotifications() {
        ...
        mClearButton.setVisibility(View.VISIBLE);
        mClearButton.setAlpha(0f);
        setAreThereNotifications(); // this will show/hide the button as
                                    // necessary
    ...
    void makeExpandedInvisible() {
        if (SPEW)
            Slog.d(TAG, "makeExpandedInvisible: mExpandedVisible=" + mExpandedVisible
                    + " mExpandedVisible=" + mExpandedVisible);
     ...
     * When layers are enabled, this method attempts to enable layers for the
     * minimal number of children. Only children visible when the notification
     * area is fully expanded will receive a layer. The technique used in this
     * method might cause more children than necessary to get a layer (at most
     * one extra child with the current UI.)
     * 
     * @param layerType {@link View#LAYER_TYPE_NONE} or
    ...
    public void showCling() {
        ...
        mHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                makeExpandedVisible(true); // enforce visibility in case the
                                           // shade is still animating closed
                animateExpandNotificationsPanel();
    ...
    public boolean interceptTouchEvent(MotionEvent event) {
        ...
        // The cling is supposed to show the first time you drag, or even tap,
        // the status bar.
        // It should show the notification panel, then fade in after half a
        // second, giving you
        // an explanation of what just happened, as well as teach you how to
        // access quick
        // settings (another drag). The user can dismiss the cling by clicking
        // OK or by
    ...
    public void updateExpandedViewPos(int thingy) {
        if (DEBUG)
            Slog.v(TAG, "updateExpandedViewPos");

        // on larger devices, the notification panel is propped open a bit
        mNotificationPanel.setMinimumHeight(
                (int) (mNotificationPanelMinHeightFrac * mCurrentDisplaySize.y));

        FrameLayout.LayoutParams lp = (FrameLayout.LayoutParams) mNotificationPanel
                .getLayoutParams();
    ...
    void updateDisplaySize() {
        ...
        if (DEBUG_GESTURES) {
            mGestureRec.tag("display",
                    String.format("%dx%d", mDisplayMetrics.widthPixels,
            ...
            synchronized (mNotificationData) {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        final int velocity = width * 8; // 1000/8 = 125 ms
                                                        // duration
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_CLOSE_SYSTEM_DIALOGS.equals(action)) {
                ...
                if (mNavigationBarView != null && mNavigationBarView.mDelegateHelper != null) {
                    // if We are in Landscape/Phone Mode then swap the XY
                    // coordinates for NaVRing Swipe
                    mNavigationBarView.mDelegateHelper
                            .setSwapXY((
                ...
                if (mGesturePanel != null) {
                    mGesturePanel
                            .setSwapXY((mContext.getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE)
                                    && (mCurrentUIMode == 0));
                    hideNavBar(); // Reset the Gesture window to the new
                                  // orientation.
                }
            }
            else if (Intent.ACTION_SCREEN_ON.equals(action)) {
                // work around problem where mDisplay.getRotation() is not
    ...
    void updateResources() {
        ...
        if (newTheme != null &&
            ...
            if (mClearButton instanceof TextView) {
                ((TextView) mClearButton).setText(context
                        .getText(R.string.status_bar_clear_all_button));
            }
    ...
    protected void loadDimens() {
        ...
        mFlingGestureMaxOutputVelocityPx = res
                .getDimension(R.dimen.fling_gesture_max_output_velocity);

        mNotificationPanelMarginBottomPx = (int) res
                .getDimension(R.dimen.notification_panel_margin_bottom);
        ...
        mNotificationHeaderHeight = res
                .getDimensionPixelSize(R.dimen.notification_panel_header_height);

        mNotificationPanelMinHeightFrac = res.getFraction(
                R.dimen.notification_panel_min_height_frac, 1, 1);
    ...
    protected void updateSettings() {
        ...
        if (mClockActions[doubleClick] == null || mClockActions[doubleClick].equals("")
                || mClockActions[doubleClick].equals("**null**")) {
        ...
        mAutoHideTimeOut = Settings.System.getInt(cr, Settings.System.NAV_HIDE_TIMEOUT,
                mAutoHideTimeOut);
        if (mNavBarAutoHide) {
            setupAutoHide();
