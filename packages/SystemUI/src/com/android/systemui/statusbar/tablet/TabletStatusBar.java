...
public class TabletStatusBar extends BaseStatusBar implements
    ...
    float mNavAreaLand = 45f;
    float mNotifAreaLand = 45f;
    float mNavAreaPort = 60f;
    float mNotifAreaPort = 30f;
...
//        lp.windowAnimations = com.android.internal.R.style.Animation_ZoomButtons; // simple fade
    ...
    public void UpdateWeights(boolean landscape) {
        ...
        mNavAreaLand = (30 + (mNumberOfButtons * 2));
        mNotifAreaLand = (90 - mNavAreaLand);
        mNavAreaPort = (35 + (mNumberOfButtons * 3));
        mNotifAreaPort = (90 - mNavAreaPort);
        if (landscape) {
            ...
            // we are using this should be helpful in phones trying to use hybrid which just 3-4 buttons
                mNavigationArea.setLayoutParams(new LinearLayout.LayoutParams(0,LayoutParams.MATCH_PARENT,mNavAreaLand));
                mNotificationHolder.setLayoutParams(new LinearLayout.LayoutParams(0,LayoutParams.MATCH_PARENT,mNotifAreaLand));
        } else {
                mNavigationArea.setLayoutParams(new LinearLayout.LayoutParams(0,LayoutParams.MATCH_PARENT,mNavAreaPort));
                mNotificationHolder.setLayoutParams(new LinearLayout.LayoutParams(0,LayoutParams.MATCH_PARENT,mNotifAreaPort));
        }
