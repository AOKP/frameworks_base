...
public class Clock extends TextView implements DemoMode {
    ...
    private final CharSequence getSmallTime() {
        ...
        if (mDateStyle != DATE_STYLE_NORMAL) {
            if (todayDate != null) {
                if (mDateStyle == DATE_STYLE_GONE) {
                    formatted.delete(result.indexOf(todayDate), result.lastIndexOf(todayDate)+todayDate.length());
                } else {
                    if (mDateStyle == DATE_STYLE_SMALL) {
                        if (todayIs != null && mWeekdayStyle == WEEKDAY_STYLE_SMALL) {
