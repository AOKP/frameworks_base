...
public class NetworkToggle extends StatefulToggle {
    ...
    String[] mMobileNetworks = new String[0];
    ...
    private void getSwitchableModes() {
        ...
        if (isDeviceGSM()) {
            default_modes = deviceSupportsGSMLTE() ? "9|0|1" : "0|1";
        } else if (isDeviceCDMA()) {
            default_modes = deviceSupportsCDMALTE() ? "8|4|5" : "4|5";
        }
        if (!default_modes.equals("")) {
            String toggles_string = Settings.AOKP.getString(mContext.getContentResolver(),
                Settings.AOKP.NETWORK_MODES_TOGGLE);
            toggles_string = toggles_string.equals("") ? default_modes : toggles_string;
            mMobileNetworks = toggles_string.split("|");
        }
    }

    private int getNextNetworkMode(int current) {
        int position = 0;
        for (int i = 0; i < mMobileNetworks.length; i++) {
            if (current == Integer.parseInt(mMobileNetworks[i])) {
                position = i;
                break;
            }
        }
        return (position + 1) == mMobileNetworks.length ? Integer.parseInt(mMobileNetworks[0]) :
                Integer.parseInt(mMobileNetworks[position + 1]);
    }
