...
public abstract class BaseToggle
    ...
    protected void updateView() {
        if (mStyle == XzibitToggler.STYLE_SWITCH) {

        } else if (mStyle == XzibitToggler.STYLE_TILE) {

            if (mLabel != null) {
                mLabel.setText(mLabelText);
                mLabel.setVisibility(View.VISIBLE);
                if (mIconDrawable != null) {
                    mLabel.setCompoundDrawablesWithIntrinsicBounds(null, mIconDrawable, null, null);
                }
                mLabel.setTextSize(1, mTextSize);
            }

        } else if (mStyle == XzibitToggler.STYLE_TRADITIONAL) {
            if (mIcon != null) {
                if (mIconDrawable != null) {
