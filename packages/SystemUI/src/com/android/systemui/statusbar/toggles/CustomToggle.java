...
public class CustomToggle extends BaseToggle {
    ...
    private void findState() {
        startMagicTricks();
        mCustomState++;
        if (mNumberOfActions >= mCustomState) {
            mCustomState = 0;
        }
    }
    ...
    private void startMagicTricks() {
        ...
        AwesomeAction.getInstance(mContext).launchAction(mClickActions[mCustomState]);
        if (iconUri != null && iconUri.length() > 0) {
            ...
            myIcon = AwesomeAction.getInstance(mContext).getIconImage(mClickActions[mCustomState]);
        }
    setLabel(toggleText);
    setIcon(myIcon);
    scheduleViewUpdate();
    ...
    public boolean onLongClick(View v) {
        AwesomeAction.getInstance(mContext).launchAction(mLongClickAction);
        return true;
    }
    ...
    private void updateSettings() {
        ...
        mNumberOfActions = Settings.System.getInt(resolver,
                Settings.System.CUSTOM_TOGGLE_QTY, 2);
