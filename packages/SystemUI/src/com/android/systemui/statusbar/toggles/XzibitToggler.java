...
public class XzibitToggler {
    ...
    public void setupTiles(QuickSettingsContainerView container) {
        ...
        if (mContainer != null) {
            Integer style = new Integer(BaseToggle.STYLE_TILE);
            setupToggles(style);
            populateContainer(style);
        }
    }

    public void setupTraditional(LinearLayout container) {
        boolean useAltButtonLayout = false;
        int widgetsPerRow = 8;
        View toggleSpacer;

        mContainer = container;
        if (container != null) {
            Integer style = new Integer(BaseToggle.STYLE_TRADITIONAL);
            setupToggles(style);
            ...
            if (!useAltButtonLayout) {
                ...
                container.setPadding(container.getPaddingLeft(), pixels,
            ...
            if (!useAltButtonLayout && (mToggles.size() % 2 != 0)) {
                // we are using switches, and have an uneven number - let's add
                // a
                // spacer
                toggleSpacer = new LinearLayout(mContext);
                rows.get(rows.size() - 1).addView(toggleSpacer, PARAMS_TOGGLE);
            }
            if (useAltButtonLayout) {
    ...
    private void setupToggles(int style) {
    ...
    private void populateContainer(int style) {
        mContainer.removeAllViews();
        for (BaseToggle toggle : mToggles) {
            mContainer.addView(toggle.createView());
        }
    }
    ...
    private void updateSettings() {
        ...
        if (mContainer != null && mContainer instanceof QuickSettingsContainerView) {
            ((QuickSettingsContainerView) mContainer).setColumnCount(columnCount);
        }

        if (mContainer != null) {
            setupToggles(mStyle);
            populateContainer(mStyle);
        }
