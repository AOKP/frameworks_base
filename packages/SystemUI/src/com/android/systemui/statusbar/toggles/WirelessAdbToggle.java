...
public class WirelessAdbToggle extends StatefulToggle {
    ...
    public void updateView() {
        ...
        if (mWifiMgr.isWifiEnabled()) { /** Check isWifiEnabled() then check if port is open
                                            i.e. when user toggles to ON state */
            if (System.getProperty("service.adb.tcp.port") == "5555") {
                enabled = true;
            } else {
                enabled = false;
            }
