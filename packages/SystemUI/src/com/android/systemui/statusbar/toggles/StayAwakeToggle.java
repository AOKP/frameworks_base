...
public class StayAwakeToggle extends StatefulToggle {
    ...
    protected void init(Context c, int style) {
        ...
        SharedPreferences p = mContext
                .getSharedPreferences(KEY_USER_TIMEOUT, Context.MODE_PRIVATE);
        currentTimeout = p.getInt("state", FALLBACK_SCREEN_TIMEOUT_VALUE);
    }
    ...
    protected void toggleInsomnia(boolean state) {
        if (state) {
            ...
            if (currentTimeout == neverSleep) { // in the case the user managed to confuse the toggle
    ...
    private void commitTimeout() {
        SharedPreferences p = mContext
                .getSharedPreferences(KEY_USER_TIMEOUT, Context.MODE_PRIVATE);
        p.edit().putInt("state", currentTimeout).commit();
    }
