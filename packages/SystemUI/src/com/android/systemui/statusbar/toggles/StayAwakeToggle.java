...
public class StayAwakeToggle extends StatefulToggle {
    ...
    private int currentTimeout, storedUserTimeout, longTimeout;
    ...
    public boolean onLongClick(View v) {
       collapseStatusBar();
       // Check if user has enabled insomnia previously and if not store it
       if (storedUserTimeout == 0) {
           storedUserTimeout = currentTimeout;
       }
       longTimeout = Settings.System.getInt(mContext.getContentResolver(),
                        Settings.System.SCREEN_OFF_TIMEOUT_LONG, FALLBACK_SCREEN_TIMEOUT_VALUE);
       setEnabledState(true);
       setTile(true);
       Settings.System.putInt(mContext.getContentResolver(),
               Settings.System.SCREEN_OFF_TIMEOUT, longTimeout);
       // Inform User of new timeout policy
       Toast.makeText(mContext,
            "Screen Timeout set to: " + String.valueOf(longTimeout / 60000)+ " minute(s)" ,
             Toast.LENGTH_LONG).show();
       mHandler.postDelayed(mRunnable, longTimeout);
    ...
    private void setTile(boolean state) {
        setLabel(state ? R.string.quick_settings_stayawake_on
               : R.string.quick_settings_stayawake_off);
       setIcon(state ? R.drawable.ic_qs_stayawake_on : R.drawable.ic_qs_stayawake_off);
    }
