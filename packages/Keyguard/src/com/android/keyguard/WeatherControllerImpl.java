...
public class WeatherControllerImpl implements WeatherController {
    ...
    private Uri mWeatherTemperatureUri;
    ...
    public WeatherControllerImpl(Context context) {
        ...
        mContext.getContentResolver().registerContentObserver(mWeatherTemperatureUri, true,
    ...
    private void queryWeather() {
        ...
        if (c == null) {
            ...
            try {
                ...
                mCachedInfo.wind = c.getString(4);
                mCachedInfo.conditionCode = c.getInt(5);
                mCachedInfo.conditionDrawable = getIcon(mCachedInfo.conditionCode);
                mCachedInfo.humidity = c.getString(6);
            } finally {
                c.close();
            }
        }
        ...
        public void onChange(boolean selfChange, Uri uri) {
            if (uri != null) {
                if (uri.compareTo(WeatherContract.WeatherColumns.CURRENT_WEATHER_URI) == 0) {
                    queryWeather();
                    fireCallback();
                } else if (uri.compareTo(mWeatherTemperatureUri) == 0) {
                    queryWeatherTempUnit();
