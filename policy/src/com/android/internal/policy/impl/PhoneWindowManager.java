...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            for (int i = 0; i < 3; i++) { // watch all 5 settings for changes.
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        final boolean screenOn = mPowerManager.isScreenOn();
     ...
     if (mCustomSoftKeysEnabled && !mForceDefault) {
        ...
        //if not down... must be up.
            ...
            if (!longPress && !keyguardOn && mDoubleClickSoftKeys) {
                switch (keyCode) {
                case KeyEvent.KEYCODE_HOME:
                    if (mDoubleClicked == 2) {
                        ...
                        if (!screenOn) {
                            launchSoftKeyAction(ACTION_POWER);
                            mDoubleClicked = 0;
                        } else {
                            strSingleClick = mHomeActions[0];
                            mHandler.postDelayed(buttonSingleClick,400);
                        }
                    }
                    break;
                case KeyEvent.KEYCODE_MENU:
            ...
            return -1;
            /* Double clicks are disabled just launch the single click */
            } else if (!longPress && !keyguardOn && !mDoubleClickSoftKeys) {
                switch (keyCode) {
                case KeyEvent.KEYCODE_HOME:
                        if (!screenOn) {
                            launchSoftKeyAction(ACTION_POWER);
                        } else {
                            launchSoftKeyAction(mHomeActions[0]);
                        }
                        break;
                case KeyEvent.KEYCODE_MENU:
                     launchSoftKeyAction(mMenuActions[0]);
                     break;
                case KeyEvent.KEYCODE_SEARCH:
                     launchSoftKeyAction(mSearchActions[0]);
                     break;
                case KeyEvent.KEYCODE_APP_SWITCH:
                     launchSoftKeyAction(mRecentsActions[0]);
                     break;
                case KeyEvent.KEYCODE_BACK:
                     launchSoftKeyAction(mBackActions[0]);
                     break;
                default:
                }
            return -1;
