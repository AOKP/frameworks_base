...
public class KeyguardSelectorView extends LinearLayout implements KeyguardSecurityView {
    ...
    private final int TORCH_LONGPRESS = ViewConfiguration.getLongPressTimeout();
    private final int TORCH_TIMEOUT = 400;
    ...
    private boolean mGlowTorchStarted;
    ...
    private void launchAction(String action) {
        ...
        if (mGlowTorch) {
            ...
            mHandler.postDelayed(fireTorch, TORCH_TIMEOUT);
        }
        switch (AwesomeEnum) {
        case ACTION_UNLOCK:
    ...
    OnTriggerListener mOnTriggerListener = new OnTriggerListener() {
       ...
       final Runnable SetLongPress = new Runnable () {
        ...
        public void onGrabbed(View v, int handle) {
            ...
            if (mGlowTorch) {
                mHandler.postDelayed(startTorch, TORCH_LONGPRESS);
            }
        }
        ...
        public void onTargetChange(View v, int target) {
            if (target == -1) {
                ...
                if ((target >= 0) && (target <= 8) && mGlowTorch) {
                    mHandler.removeCallbacks(startTorch);
                    mHandler.postDelayed(fireTorch, TORCH_TIMEOUT);
                }
    ...
    final Runnable startTorch = new Runnable () {
        public void run() {
            boolean torchActive = Settings.System.getBoolean(
                    mContext.getContentResolver(), Settings.System.TORCH_STATE, false);

            if (!mGlowTorchStarted && !torchActive) {
                mGlowTorchStarted = true;
    ...
    final Runnable fireTorch = new Runnable () {
        public void run() {
            if (mGlowTorchStarted) {
                mGlowTorchStarted = false;
                Intent intentTorch = new Intent("android.intent.action.MAIN");
                intentTorch.setComponent(ComponentName.unflattenFromString("com.aokp.Torch/.TorchActivity"));
                intentTorch.addCategory("android.intent.category.LAUNCHER");
