...
import com.android.internal.widget.GfxEngineRelayService;
    ...
    private class VerifyRunner implements Runnable {
        public void run() {
            ...
            try {
                while (true) {
                    if (m_bPaused) {
                        ...
                        if (AM_STATUS.getDeclaredField("eAM_STATUS_OK").getInt(AM_STATUS) == iResult) {
                    ...
                    if (AM_STATUS.getDeclaredField("eAM_STATUS_CREDENTIAL_LOCKED").getInt(AM_STATUS) == iResult) {
                        ...
                        //runOnUiThread(new Runnable() {
                            ...
                            //}
                        //});
                    } else if (AM_STATUS.getDeclaredField("eAM_STATUS_USER_CANCELED").getInt(AM_STATUS) == iResult) {
                        if (m_bStartAgain) {
            ...
            try {
                if (iResult == AM_STATUS.getDeclaredField("eAM_STATUS_OK").getInt(AM_STATUS)) {
                ...
                } else if (iResult == AM_STATUS.getDeclaredField("eAM_STATUS_CREDENTIAL_LOCKED").getInt(AM_STATUS)) {
                    // When m_bPaused becomes true.
                    bRetryAfterLockout = true;
                } else if (iResult == AM_STATUS.getDeclaredField("eAM_STATUS_NO_STORED_CREDENTIAL").getInt(AM_STATUS)) {
                    // Should never happen.
                    ...
                    bRetryAfterLockout = true;
                } else if (iResult == AM_STATUS.getDeclaredField("eAM_STATUS_UI_TIMEOUT").getInt(AM_STATUS)) {
                    ...
                    bRetryAfterLockout = true;
                } else if (iResult == AM_STATUS.getDeclaredField("eAM_STATUS_UNKNOWN_ERROR").getInt(AM_STATUS)) {
