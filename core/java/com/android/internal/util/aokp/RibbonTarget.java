...
public class RibbonTarget {
    ...
    public RibbonTarget(Context context, final String sClick, final String lClick, final String cIcon, final boolean text, final int size) {
        mContext = context;
        ...
        if (!cIcon.equals("**null**")) {
            if (size > 0) {
                mIcon.setImageDrawable(resize(getCustomDrawable(mContext, cIcon), mapChosenDpToPixels(size)));
            } else {
                mIcon.setImageDrawable(getCustomDrawable(mContext, cIcon));
            }
        } else {
            if (size > 0) {
                mIcon.setImageDrawable(resize(NavBarHelpers.getIconImage(mContext, sClick), mapChosenDpToPixels(size)));
            } else {
