    ...
    public static ScrollView getGridView(Context mContext, ArrayList<String> apps, int color, int columns) {
        ...
        for (int i = 0; i < intDiv; i++) {
            ...
            for (int j = 0; j < columns; j++) {
                row.addView(targets.get(0).getView(), PARAMS_TARGET_SCROLL);
                targets.remove(0);
            }
            table.addView(row, PARAMS_TARGET_SCROLL);
        }
        length = targets.size();
        LinearLayout newRow = new LinearLayout(mContext);
        newRow.setOrientation(LinearLayout.HORIZONTAL);
        for (int i = 0; i < length; i++) {
            newRow.addView(targets.get(i).getView(), PARAMS_TARGET_SCROLL);
        }
        newRow.setGravity(Gravity.CENTER);
        table.addView(newRow, PARAMS_TARGET_SCROLL);
        appGridView.addView(table, PARAMS_TARGET_SCROLL);
        return appGridView;
    }
}