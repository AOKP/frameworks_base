...
public class AokpRibbonHelper {
    ...
    private static final LinearLayout.LayoutParams PARAMS_TARGET = new LinearLayout.LayoutParams(
            LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT, 1f);

    private static final LinearLayout.LayoutParams PARAMS_TARGET_VERTICAL = new LinearLayout.LayoutParams(
    ...
    public static ScrollView getVerticalRibbon(Context mContext, String shortTargets, String longTargets, boolean text, int size) {
        ...
        if (!TextUtils.isEmpty(shortTargets) && !TextUtils.isEmpty(longTargets)) {
            ...
            targetsLayout.setGravity(Gravity.CENTER_VERTICAL);
            targetScrollView.setOverScrollMode(View.OVER_SCROLL_IF_CONTENT_SCROLLS);
            for (int i = 0; i < targets.size(); i++) {
                targetsLayout.addView(targets.get(i).getView(), PARAMS_TARGET);
            }
            targetScrollView.addView(targetsLayout, PARAMS_TARGET_VERTICAL);
        }
        return targetScrollView;
    }
}